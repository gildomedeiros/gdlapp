...
public final class AimingCycleLogTest {
    ...
    public static void main(String[] args) throws Exception {
        ...
        log.enable();
        AimingSessionTest.Fake f=new AimingSessionTest.Fake(); f.nearby=true; f.aiming();
        ...
        f.input=used; f.core.observeDirection(used,f.time); f.core.tick();
        ...
        AimingCycleLog.record(log,f.core,other,f.time,true,false,f.core.cycleId,f.sent.get(f.sent.size()-1));
        f.time+=100; f.input=new AimingSession.Inputs(new AimingSession.Fix(0,0,Double.NaN,f.time,1235),0,0,0,f.time,null,true);
        f.core.tick(); AimingCycleLog.record(log,f.core,f.input,f.time,true,false,f.core.cycleId,0);
        f.time+=100; f.input=new AimingSession.Inputs(fix,0,0,0,f.time,"pilot_stick",true);
        f.core.tick(); AimingCycleLog.record(log,f.core,f.input,f.time,true,false,-1,7);
        ...
        check(text.contains("\"lockedDirection\":\"right\""),"lock recorded");
