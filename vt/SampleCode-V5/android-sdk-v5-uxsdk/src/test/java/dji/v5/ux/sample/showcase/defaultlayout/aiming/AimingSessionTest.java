...
public final class AimingSessionTest {
    ...
    public static void main(String[] args) {
        ...
        // CAM3 v2.7: Retain accuracy bypass and verify removal of minimum-distance rejection.
        check(YawAimingMath.isBearingUsable(20, 10), "valid accuracy at 20 m accepted");
        check(YawAimingMath.isBearingUsable(4.99, 3), "v2.7 below 5 m allowed");
        ...
        for (String cause : new String[]{"gps", "gps_quality", "stale_gps", "heading", "aircraft_gps", "hover"}) {
        ...
        for (String cause : new String[]{"user_stop", "landing", "returning_home"}) {
            f = new Fake(); f.aiming(); f.core.pauseImmediately("stale_gps"); f.core.tick();
        ...
        f = new Fake(); f.aiming(); f.input = f.data(f.time, f.time, "stale_gps", true); f.core.tick();
        f.owner = new AimingSession.Authority(AimingSession.Owner.MSDK, false, false); f.core.tick();
        check(f.core.state() != AimingSession.State.PAUSED, "control loss outranks continuing stale GPS failure");
        f = new Fake(); f.aiming(); f.core.pauseImmediately("stale_gps"); f.core.tick(); f.advance(501);
        ...
        f = new Fake(); f.core.startAiming(); f.core.pauseImmediately("stale_gps"); f.core.tick();
        for (int i = 0; i < 51; i++) { f.time += 100; f.input = f.data(f.time, f.time, "stale_gps", true); f.core.tick(); }
