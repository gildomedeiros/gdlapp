...
public final class MainActivity extends Activity {
    ...
    private Button phoneStart,wifiStart;
    private TextView wifiStatus,wifiPosition,wifiFiles,wifiCounters,wifiRecent,wifiAge;
    ...
    private final Runnable redraw=new Runnable() {
        public void run() {
            ...
            wifiStatus.setText(WifiCaptureService.status);
            wifiPosition.setText(WifiCaptureService.telemetry);
            wifiFiles.setText(WifiCaptureService.file.isEmpty()?"Automatic files: lora_tester_lora_wifi_….kml + .jsonl":WifiCaptureService.file);
            wifiCounters.setText(WifiCaptureService.counters);
            wifiRecent.setText(WifiCaptureService.recent);
            long wifiLast=WifiCaptureService.lastReading;
            wifiAge.setText(wifiLast==0?"No Wi-Fi readings yet":String.format(Locale.US,"Last Wi-Fi reading %.1f seconds ago",(SystemClock.elapsedRealtime()-wifiLast)/1000.0));
            wifiStart.setEnabled(!WifiCaptureService.active);
    ...
    public void onCreate(Bundle saved) {
        ...
        start=new Button(this); start.setText("Start LoRa USB"); actions.addView(start,new LinearLayout.LayoutParams(0,dp(54),1));
        ...
        label(body,"LORA WI-FI — TTGO");
        TextView wifiHelp=text("Connect phone Wi-Fi to GDL_LORA first (password: gdl12345). Keep the connection if Android says no internet. Stop the Python test while using this app: TTGO forwards to one client at a time.",13);body.addView(wifiHelp);
        Button settings=new Button(this);settings.setText("Open Wi-Fi settings");body.addView(settings);
        settings.setOnClickListener(v->startActivity(new Intent(android.provider.Settings.ACTION_WIFI_SETTINGS)));
        LinearLayout wifiActions=new LinearLayout(this);
        wifiStart=new Button(this);wifiStart.setText("Start LoRa Wi-Fi");wifiActions.addView(wifiStart,new LinearLayout.LayoutParams(0,dp(64),1));
        Button wifiStop=new Button(this);wifiStop.setText("Stop Wi-Fi");wifiActions.addView(wifiStop,new LinearLayout.LayoutParams(0,dp(64),1));body.addView(wifiActions);
        wifiStart.setOnClickListener(v->startForegroundService(new Intent(this,WifiCaptureService.class)));
        wifiStop.setOnClickListener(v->{if(WifiCaptureService.active)startService(new Intent(this,WifiCaptureService.class).setAction("STOP"));});
        wifiStatus=text("",15);body.addView(wifiStatus);wifiPosition=card(body,"",17);
        wifiAge=text("",13);body.addView(wifiAge);wifiCounters=card(body,"",14);wifiFiles=card(body,"",12);wifiFiles.setTextIsSelectable(true);
        wifiRecent=card(body,"",12);wifiRecent.setTextIsSelectable(true);
        ...
        label(body,"USB — LATEST TELEMETRY"); position=card(body,"Waiting for GPS readings",19);
        ...
        TextView note=text("All sources save Google Earth KML plus raw JSONL automatically. Open both KML files to compare: phone is blue, LoRa is orange. Phone RSSI/SNR are unavailable; HDOP depends on phone support. Close other USB terminal apps before connecting.",13); note.setTextColor(0xff9bafc2); body.addView(note);
